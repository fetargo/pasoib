...
def relogined(reload):
    cur = con.cursor()
    cur.execute('UPDATE "Access" SET "access" = %(rld)s' +
                ' WHERE "access" NOT LIKE %(rld)s', {'rld':reload})
    try:
        con.commit()
        print("Reload state updated")
        print("Login.relogined return code: 0")
        return 0
    except:
        print("Reload state update failed")
        print("Login.relogined return code: 1")
        return 1
